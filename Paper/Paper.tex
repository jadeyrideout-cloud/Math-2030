...
\title{Dominoes: Complexity from Simplicity}
\author{Jay Adey-Rideout\\\\\textit{Math 2030}}

\begin{document}
\maketitle
\newpage
\begin{abstract}
European dominoes and their surrounding games have been left out of many mathematical discussions. This is due to the lack of complexity in the standard tournament rules, though those rules can be changed. We set out to find a new rule set based off this standard that adds complexity to the patterns formed by the final board state of the game, by devising a complexity metric. Three different rule sets were created, and data was gathered through gameplay trials with real participants. The strategy and enjoyability of each rule set was also taken into account, as to allow further research into game theory and statistics.
\end{abstract}
\newpage
\section{Introduction}
The standard European domino originates from Italy, and was originally made from ivory and sometimes ebony \cite{Rules}. Modern dominoes are usually made of porcelain, as are the set used in our research. European dominoes are rectangular, with dimensions of \(1\times2\). Each end of the domino holds a numerical value in the number of pips, divided by a centre line as seen in the figure below.
\begin{figure}[h!]
    \centering
...
Current literature on European dominoes is rather scattered, being in many different fields. The bulk of studies use the form factor of a domino for other purposes, such as an educational tool. Assigning facts, questions, or simple problems to each end of a piece has been found to be a useful learning aid for younger students \cite{education}. Mathematical literature usually takes one of two forms. The most common is to define a game with similar rules but different pieces \cite{squares}. As seen below, many use squares with multicoloured edges for ease of tiling the plane.
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.2\linewidth]{photos/Square.png}
    \caption{A domino similar to that described by Chlebus \cite{squares}.}
    \label{Square}
\end{figure}
\\These dominoes are most often used in discussion of tiling or machine learning \cite{turing}. The other common form of mathematical research is the strategy of one specific regional variant of the game \cite{regional}. Most of these games use European dominoes, though the regionality of the rule set limit both reach and application. Our research aims to open an alley for a more standardised discussion on both of these topics.
...
\end{enumerate}
The first of these questions is fairly self explanatory, as rules can be added and changed to create different patterns. Question two on the other hand is qualitative in nature, and we believe it to be just as important. In our eyes, further research into a traditional game that is not enjoyable to participate in is a waste of both time and energy, and thus we will attempt to find a rule set that strikes a balance between these two factors. Both these questions are answered in section 3.5.
\pagebreak
\section{Methods}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.4\linewidth]{photos/A8(1) final.png}
...
Rule set two gains all the rules of set I, adding the ability to play pieces at a ninety degree angle from the previous as seen in figure \ref{gameB}.
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.4\linewidth]{photos/B7 final.png}
    \caption{The end state of a game of rule set II}
    \label{gameB}
\end{figure}
\\Due to this, two new illegal moves must be defined. The first is placing a domino alongside another in parallel, simply to streamline gameplay. The second is that the minimum space between two dominoes to not be touching is \(\frac{1}{2}w\). As the dominoes are not confined to a perfect grid, some small amount of shifting can occur at each intersection. This can compound, resulting in two dominoes that should be touching not physically doing so. Due to the way doubles shift alignment of the next piece by \(\frac{1}{2}w\), the minimum space between non adjacent dominoes will be this offset.
\pagebreak
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.4\linewidth]{photos/C5 final.png}
...
\begin{figure}[h!]
    ...
    \label{graphA}
\end{figure}
\\Most metrics for rule set I stayed consistent between rounds, having the middle round length of the three. This was found by participants to be the most strategic of the three rule sets, and more engaging than Block.
\pagebreak
\subsection{Rule Set II}
...
\begin{figure}[h!]
    ...
    \label{graphB}
\end{figure}
\\Round length for this rule set is more random than the first, due to the blocking of moves. Participants found this rule set to still be somewhat strategic, as well as the most engaging due to the interesting patterns produced.
\pagebreak
\subsection{Rule Set III}
This final rule set was where the playability of the game began to fall apart. With the additional doubles created at intersections there was rarely a situation where a player had no possible moves. As seen in the winner column for session one in \ref{A3}, this lead to the player who started any given round winning it. The number of players then offsets this patten, as seen in the data for session two. Our participants found that this took most of the enjoyment out of this rule set, as the outcome was predictable. Some data from \ref{A3} is shown below:
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\linewidth]{graphs/Rules III chart.png}
    \caption{Select data from rule set III}
    \label{graphC}
\end{figure}
\\As rule set III adds more openings to place dominoes, the extra intersections and turns are expected. What is not as intuitive at first glance is the consistent number of pieces placed, though this is a bi-product of the predictability explained above. Overall, this rule set was found by participants to be unenjoyable, with little strategy and predictable win conditions.
\pagebreak
\subsection{Complexity}
...
\begin{figure}[h!]
    ...
    \label{graphcom}
\end{figure}
\\As seen, average \(C\) values increase with each rule set. This increase appears to be linear, though no concrete conclusion can be drawn from such a limited data set. The maximum bound of \(C\) increases drastically between each rule set as is expected with the raising rule set, though the minimum bounds show an interesting finding. The lowest bound for both rule set I and II is zero, meaning at least one round was identical in play to a round of Block. This is due to the limited number of move options in any given game state in these two rule sets, giving a random chance of no player being able to branch out from the form of Block. This chance is only slightly lowered in rule set III, as seen by the increased minimum bound.

\subsection{Overall Analysis}
Through a culmination of all the above findings, our main questions can be answered. The simplest answer to question one is that rule set III gives the most complexity, though this changes when taking into account question two. Participants found rule set III to be the least enjoyable, showing that the rule changes therein answer question two. This leaves rule set II, which strikes the best balance between playability and complexity, answering question one. An example of the complexity is shown in figure \ref{square} on the following page, one of the more interesting patterns to come out of rule set II.
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{photos/B2 final.jpg}
    \caption{An odd pattern from rule set II}
    \label{square}
\end{figure}
\pagebreak
\section{Conclusion}
European dominoes, and the games played with them, have withstood the test of time for an extraordinary period for such a simple idea. Though due to this simplicity mathematical research is few and far between, and we set out to change that. With the creation of three variations on tournament rules, we collected data on real games to find an equilibrium of mathematical complexity and strategy. Complexity was compared via equation \ref{Eqn1}, with strategy coming from the testimonials of participants. We found that rule set II struck the best balance of these two characteristics, with it being the favourite amongst participants. We hope that these findings can open doors to expand research into European dominoes to the fields of tessellation, statistics, and game theory just to name a few.

\section*{Acknowledgements}
We would like to acknowledge several people if not for whom this research would not have been possible. Our biggest thank you goes to Kaitlin Healey, along with Tiffany Anne and Nicole Regier of the MUN Folklore Society for participation in data collection. Until undertaking this research we were not aware gathering data could be entertaining. The domino set used for data collection was a childhood set graciously donated by Lisa Adey-Rideout, which we appreciate greatly. A thank you also goes to Logan Sacrey, for being our first introduction to the wonders of \LaTeX.
\pagebreak
\section*{Appendix}
\appendix
