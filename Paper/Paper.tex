...
\newpage
\begin{abstract}
European dominoes and their surrounding games have been left out of many mathematical discussions. This is due to the lack of complexity in the standard tournament rules, though those rules can be changed. We set out to find a new rule set based of the standard that adds complexity to the patterns formed by the final board state of the game, by devising a complexity metric. Three different rule sets were created, and data was gathered through gameplay trials with real participants. The strategy and enjoyability of each rule set was also taken into account, as to allow further research into game theory and statistics.
\end{abstract}
\newpage
...
Through a culmination of all the above finding, our main questions can be answered. The simplest answer to question one is that rule set III gives the most complexity, though this changes when taking into account question two. Participants found rule set III to be the least enjoyable, showing that the rule changes there in answer question two. This leaves rule set II, which strikes the best balance between playability and complexity, answering question one. An example of the complexity is shown in figure \ref{square} below, one of the more interesting patterns to come out of rule set II.
\begin{figure}
    \centering
...
\pagebreak
\section{Conclusion}
European dominoes, and the games played with them, have withstood the test of time for an extraordinary period for such a simple idea. Though due to this simplicity mathematical research is few and far between, and we set out to change that. With the creation of three variations on tournament rules, we collected data on real games to find an equilibrium of mathematical complexity and strategy. Complexity was compared via equation \ref{eqn2}, with strategy coming from the testimonials of participants. We found that rule set II struck the best balance of these two characteristics, with it being the favourite amongst participants. We hope that these finding can open doors to expand research into dominoes to the fields of tessellation, statistics, and game theory just to name a few.
